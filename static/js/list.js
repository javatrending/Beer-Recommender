...
d3.json('/run', function (data) {
        ...
        data.forEach(indx => {
            ...
            if (indx.beer_name == chosenBeers[0] || indx.beer_name == chosenBeers[1] || indx.beer_name == chosenBeers[2]) {
                ...
                if (indx.cluster == i) {
                    ...
                    // style_stats['cluster_beer_count'] = indx.cluster_beer_count;
                    ...
                    group_stats['cluster_beer_count'] = indx.cluster_beer_count;
    ...
    d3.selectAll('#rate').nodes().forEach((rect, i) => {
        d3.select(rect).selectAll('text')
            .data([Object.values(top_beers)[i]]).enter()
            .append('text')
            .text(parseFloat(Object.values(top_beers)[i][1].ratings.group_overall))
            .attr('text-anchor', 'middle')
...
function calculateClusterBeerCounts(data) {

    var totals = {};
    Object.entries(data.styles).forEach(d => {
        totals[d[0]] = d[1].main_style_cluster_count;
    })
...
function createScatter(data, chosenValue) {
    ...
    var total_cluster = calculateClusterBeerCounts(data);
    ...
    set_styles.forEach((style, i) => {
        ...
        if (data.styles[style]) {
            ...
            value_list['total'] = (+data.styles[style].main_style_cluster_count / total_cluster) * 100;
...
function rollOver(circleGroup, totals, label, value) {
    ...
    var toolTip = d3.tip()
        ...
        .offset([115, 115])
        .html(function (d, i) {
            return (`${label} Review (Cluster): ${d[value]}<br>Style Percentage (Cluster): ${(d.total).toFixed(2)}%<br>${label} Review (Full Set): ${d.all[value]}<br>Style Percentage (Full Set): ${(d.all.style_total/d.all.overall_total * 100).toFixed(2)}%`);
        });
